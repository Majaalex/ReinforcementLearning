...
class Bandit(object):
    def __init__(self, numArms, trueRewards, epsilon, mode):    # we add a mode to know tif we are trying a constant alpha of 0.1 or 1/n
    ...
    def pull(self):
        ...
        self.trueRewards = [reward + 0.1*np.random.randn() for reward in self.trueRewards] # We add a normally distributed random number with standard deviation 0.1

        return np.random.randn() + self.trueRewards[whichArm]

    def updateMean(self, sample, alpha):
        ...
        if self.mode == 'sample-average':
            ...
            self.Q[whichArm] = self.Q[whichArm] + alpha *(sample - self.Q[whichArm])

def simulate(numArms, epsilon, numPulls, mode, alpha):
    ...
    for j in range(2000):
        ...
        for i in range(numPulls):
            reward = bandit.pull()
            bandit.updateMean(reward, alpha)
